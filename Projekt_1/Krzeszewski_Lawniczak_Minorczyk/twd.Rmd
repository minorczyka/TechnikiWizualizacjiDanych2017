...
if (nrow(df3) > 0) {
  ggplot(data=df3, aes(x=delay)) +
    geom_histogram(binwidth = 120, fill="#66A7A7") +
    labs(y = "Liczba pojazdów", x = NULL, title = "Opóźnienia w okolicy") +
    scale_color_gradient2(low = "green4", mid = "yellow3", high = "red2", midpoint = 10) +
    scale_x_continuous(
      breaks = function(x) return(seq(floor(x[1] / 120) * 120, x[2], 120)),
      labels = function(x) return(paste0(as.character(x/60), "-", as.character(x/60 + 2), " min"))) +
