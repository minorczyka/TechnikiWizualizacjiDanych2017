...
map1 <- qmplot(lon, lat, data = res_delayed, maptype = "toner-lite", color=line, size=delay)

box1 <- ggplot(res_filtered, aes(x = line, y = delay, color = line)) +
...
multiplot(map1, box1, cols = 2)

# 
# qmap <- qmplot(lon, lat, data = res_delayed, maptype = "toner-lite", color=line, size=delay) +
#   geom_label_repel(aes(label = speed), size=4, nudge_x= 0.003, nudge_y = 0.003)
# qmap
