...
getMainCharacters = function(df) {
  ...
  df = count(filter(df, Person %in% mainCharacters$Person), Person)
  return(df)
}

createRelationPlot = function(df) {
  mainCharacters = getMainCharacters(df)
  df = data.frame(SourceName=df$Person, TargetName=df[c(nrow(df), 1:(nrow(df)-1)), "Person"])
  ...
  nodeList$n = nodeList$n / max(nodeList$n)
  ...
  result <- forceNetwork(Links = edgeList, # data frame that contains info about edges
                                           ...
                                           radiusCalculation = "d.nodesize*50",
                                           ...
                                           linkWidth = networkD3::JS("function(d) { return d.value*40; }"),# Function to determine link/edge thickness, uses variables already defined in forceNetwork function (not variables from a data frame)
                                           linkDistance = 400,
                                           opacity = 0.8, # opacity
